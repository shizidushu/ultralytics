...
if not resume:
    ...
    model = YOLO('yolov8n-pose.yaml', task='pose').load('yolov8n-pose.pt')
else:
    model = YOLO('runs/pose/train/weights/last.pt', task='pose')

device_count = torch.cuda.device_count()
model.train(data='coco-pose.yaml', imgsz=640, epochs=200, resume=resume, batch = 32 * device_count, device=list(range(0,device_count)))
